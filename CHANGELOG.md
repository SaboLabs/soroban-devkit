...
### Added
- **Contract provenance metadata in WASM inspection.** `sdkt wasm inspect` and the on-chain metadata path now decode `contractmetav0` key/value entries into the additive `metadata.contract_meta` JSON field and a readable `Contract Meta` block. Missing sections remain valid, while malformed XDR is reported as a classified parse error (#171).
