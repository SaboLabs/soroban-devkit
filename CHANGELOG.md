...
### Added
- **Event signature and custom type definition change detection in upgrade safety.** `sdkt diff --upgrade-safety` (and the `sdkt deploy --deny-breaking` guard built on it) compared events and custom types by name only, so a contract that changed an event's parameters or a type's members while keeping the name diffed clean and the verdict reported `compatible: true` — a false pass on a genuinely breaking change that silently breaks downstream decoders. `ContractEvent` now retains the full `EventV0` signature (`prefix_topics`, `params` with name/type/topic-vs-data location, and `data_format`) and `TypeMember` retains its member type(s) and, for enums and error enums, its case discriminant, so `SpecDiff` gains `changed_events` and `changed_types` alongside the existing `changed_functions`. New `ChangeKind` variants `ChangedEvent` and `ChangedTypeDefinition` are classified as breaking, and `sdkt diff` reports a "Changed events" / "Changed types" section with the old and new shapes. Doc-only edits are not treated as changes (#72).
