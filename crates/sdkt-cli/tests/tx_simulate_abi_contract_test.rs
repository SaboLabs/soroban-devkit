//! — `tx simulate --abi-contract` tests (hermetic).
//!
//! `sdkt tx simulate --abi-contract <id>` fetches a deployed contract's on-chain
//! WASM via `inspect_contract` -> `get_wasm_bytecode`, parses it to a
//! `ContractSpec`, and decodes the primary simulation result using
...
/// Mock JSON-RPC server covering the whole `tx simulate --abi-contract` flow:
///
...
#[test]
fn tx_simulate_help_documents_abi_contract() {
    sdkt_isolated(tempdir().unwrap().path())
        .args(["tx", "simulate", "--help"])
        .assert()
        .success()
...
#[test]
fn tx_simulate_abi_and_abi_contract_are_mutually_exclusive() {
    sdkt_isolated(tempdir().unwrap().path())
        .args([
            "tx",
            "simulate",
...
#[test]
fn tx_simulate_abi_contract_decodes_result_from_onchain_wasm() {
...
#[test]
fn tx_simulate_abi_contract_json_output_has_decoded_result() {
    let dir = tempdir().unwrap();
...
#[test]
fn tx_simulate_abi_contract_missing_contract_is_clear_error() {
    // A valid ContractNotFound: the mock serves no contract-data entry for this
    // other StrKey, so `inspect_contract` degrades to a controlled error.
    let dir = tempdir().unwrap();
    let (url, _) = mock_rpc_server_with_abi(WASM_WITH_RETURN);
    add_mock_profile(dir.path(), &url);
...
#[test]
fn tx_simulate_abi_contract_offline_is_graceful() {
...
#[test]
fn tx_simulate_without_abi_preserves_behavior() {
    // No-ABI behavior is unchanged: simulation runs, but nothing is decoded.
    let dir = tempdir().unwrap();
    let (url, _) = mock_rpc_server_with_abi(WASM_WITH_RETURN);
    add_mock_profile(dir.path(), &url);
